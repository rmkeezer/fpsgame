...
class MainMenuScreen extends MonoBehaviour
{
	...
	function DrawGUI(event : Event)
	{
        ...
        if(showProgress)
        {   
            var currentProgress : float = IndustryLoader.industryProgress;
	...
	function GetSceneRef()
	{
        ...
        else if(Application.loadedLevelName == "trenches")
        {
            currentScene = 1;
        }
...
//        var currentQualityLevel : int = QualitySettings.currentLevel;
	...
	function DrawAbout()
	{
		...
		GUILayout.Space(17);
		GUILayout.BeginVertical();
		GUILayout.Label("Developed by These People", aboutStyle, GUILayout.Width(423));
		GUILayout.Space(5);
		var names : String;
		names = "Matthew Keezer and ";
		names += "Nicolas Doss";
		GUILayout.Label(names, GUILayout.Width(400));
		GUILayout.Space(20);
		GUILayout.Label("Programming by:", aboutStyle, GUILayout.Width(423));
		GUILayout.Space(5);
		names = "Matthew Keezer";
		GUILayout.Label(names, GUILayout.Width(400));
		GUILayout.Space(20);
		GUILayout.Label("Music by:", aboutStyle, GUILayout.Width(423));
		GUILayout.Space(5);
		names = "Nicolas Doss";
		GUILayout.Label(names, GUILayout.Width(400));
            ...
            if(numofPlayers != 0) // only listing room where some action is 
            {
                ...
                if(numofPlayers < maxPlayers)
                {
                    if (GUILayout.Button("Go",conectStyle,GUILayout.Width(coRect.width/10)) )
                    {
                        ...
                        //PhotonNetwork.JoinRoom(key.Key);
		...
		if(resumeRect.Contains(mousePos))
		{
			...
			if(alpha <= 0.0 && GameManager.showMenu)
			{
				if(evt.type == EventType.MouseUp && evt.button == 0 && Time.time > lastMouseTime)
				{
					...
					visible = false;
					transform.parent.gameObject.GetComponent(GameManager).TogglePause();
					lastMouseTime = Time.time;
				}
			}
		}
